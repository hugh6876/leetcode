...
public class PalindromPartitioningII {
	...
	public int minCut(String s) {
		...
		for (int i = 2; i <= s.length(); i++) {
			...
			if(isPalindrone[0][i-1] == true){
				dp[i] = 0;
				continue;
			}
			for (int j = 1; j < i; j++) {
				if( isPalindrone[j+1-1][i-1] == true && dp[j] != Integer.MAX_VALUE ){
	...
	private boolean[][] getIsPalindrone(String s) {
		...
		for (int len = 2; len < s.length(); len++) {
			for (int start  = 0; start  + len < s.length(); start ++) {
				isPlindrone[start][start+len] = isPlindrone[start+1][start+len-1] && s.charAt(start) == s.charAt(start+len);
	...
	private void minCutHelper(String src, List<String> path, int pos,int cutNum) {
		if(pos == src.length()) {
			if(minCutNum > cutNum ) {
				minCutNum = cutNum-1;
			}
		...
		for (int i = pos+1; i <= src.length(); i++) {
			...
			path.add(prefix);
			minCutHelper(src,path,i,cutNum+1);
